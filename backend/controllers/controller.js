...
function createTransporter() {
  return nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: process.env.EMAIL_USER || 'nikhilpramod425@gmail.com',
      pass: process.env.EMAIL_PASS || 'fwaxnjpbexksyidp',
    },
  });
}

async function sendOTPEmail(to, otp, subject = 'ManMode — Your OTP Code') {
  const transporter = createTransporter();
  await transporter.sendMail({
    from: `"ManMode" <${process.env.EMAIL_USER}>`,
    to,
    subject,
    html: `
      <div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;padding:24px;background:#f8f8f8;border-radius:12px;">
        <h2 style="color:#b8860b;font-size:24px;margin-bottom:8px;">ManMode</h2>
        <h3 style="color:#111;font-size:20px;margin-bottom:16px;">${subject}</h3>
        <p style="color:#444;font-size:15px;margin-bottom:24px;">Use the OTP below. It expires in <strong>5 minutes</strong>.</p>
        <div style="text-align:center;margin:24px 0;">
          <span style="font-size:36px;font-weight:900;letter-spacing:8px;color:#b8860b;background:#fff;padding:16px 32px;border-radius:8px;border:2px solid #e0c060;display:inline-block;">${otp}</span>
        </div>
        <p style="color:#888;font-size:13px;">If you didn't request this, you can safely ignore this email.</p>
        <p style="color:#888;font-size:13px;margin-top:16px;">— ManMode Team</p>
      </div>
    `,
  });
}
  ...
  new GoogleStrategy(
    ...
    async (accessToken, refreshToken, profile, done) => {
      try {
        ...
        const picture = profile._json.picture || profile.photos?.[0]?.value || '';

        let user = await User.findOne({ email });
        if (!user) {
          user = new User({ name, email, googleImage: picture, isActive: true });
          await user.save();
        } else {
          // Always refresh Google picture
          if (picture) {
...
export const sendOTP = async (req, res) => {
    ...
    try {
        ...
        req.session.otpData = { email, otp, createdAt: Date.now() };
        await sendOTPEmail(email, otp, 'ManMode — Verify Your Email');
...
export const forgotsendOTP = async (req, res) => {
  ...
  try {
    ...
    const otp = Math.floor(100000 + Math.random() * 900000);
    req.session.otpData = { email, otp, createdAt: Date.now() };
    await sendOTPEmail(email, otp, 'ManMode — Reset Your Password');
    return res.status(200).json({ message: 'OTP sent successfully!' });
  } catch (error) {
    console.error('forgotsendOTP error:', error);
    return res.status(500).json({ error: 'Failed to send OTP. Please try again later.' });
  }
};
...
export const sendLoginOTP = async (req, res) => {
  try {
    ...
    if (!user)                  return res.status(422).json({ message: 'No account found with this email.' });
    if (!user.isActive)         return res.status(403).json({ message: 'Your account has been blocked.' });
    if (!user.password)         return res.status(422).json({ message: 'This account uses Google login. Please sign in with Google.' });
    ...
    const otp = Math.floor(100000 + Math.random() * 900000);
    req.session.loginOtpData = { email: email.toLowerCase(), otp, createdAt: Date.now() };
    await sendOTPEmail(email, otp, 'ManMode — Login Verification');
    return res.status(200).json({ message: 'OTP sent to your email!' });
  } catch (e) {
    console.error('sendLoginOTP error:', e);
...
export const loginUser = async (req, res) => {
  try {
    ...
    const loginOtp = req.session.loginOtpData;
    if (!loginOtp)
      return res.status(400).json({ message: 'Session expired. Please start login again.' });

    if (loginOtp.email !== email.toLowerCase())
      return res.status(400).json({ message: 'Email mismatch.' });

    if (Date.now() - loginOtp.createdAt > 5 * 60 * 1000)
      return res.status(400).json({ message: 'OTP expired. Please request a new one.' });

    if (String(otp).trim() !== String(loginOtp.otp).trim())
      return res.status(422).json({ message: 'Incorrect OTP. Please try again.' });

    const user = await User.findOne({ email: email.toLowerCase() });
    if (!user || !user.isActive)
      return res.status(403).json({ message: 'Account not found or blocked.' });

    // Clear OTP from session
    delete req.session.loginOtpData;
...
export const updateUserProfile = async (req, res) => {
  try {
    ...
    // Handle base64 image upload via Cloudinary
    if (image && image.startsWith('data:')) {
      try {
        const result = await cloudinary.uploader.upload(image, {
          folder: 'manmode/avatars',
    ...
    await user.save();
    res.status(200).json({ success: true, message: 'Profile updated!', image: user.image || user.googleImage || '' });
  } catch (error) {
...
// };
    ...
    export const dshboard = async (req, res) => {
      try {
        ...
        let userImage = '/images/user.webp';
        if (user?.image) {
          // Cloudinary URL (starts with https) or local path
          userImage = user.image.startsWith('http') ? user.image
            : `/uploaded-images/${user.image.replace(/\\/g,'/').replace(/^uploads\/|^\/uploads\//,'').replace('uploaded-images/','').trim()}`;
        ...
        res.json({ userImage, cartCount, wishlistCount, userName: user?.name || '', userEmail: user?.email || '' });
      } catch (error) {
...
export const changeProfilePassword = async (req, res) => {
  try {
    ...
    // Google-auth users may have no password set
    if (!user.password)
      return res.status(400).json({ error: 'Cannot set password for Google-authenticated accounts.' });
